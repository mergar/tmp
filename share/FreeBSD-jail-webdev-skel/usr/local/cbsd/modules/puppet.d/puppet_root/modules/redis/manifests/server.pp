...
define redis::server( $ensure=present,
	$version="latest",
	$redisname="redis",
	$bind="0.0.0.0",
	$port=6379,
	...
	case $operatingsystem {
		centos, redhat, debian, ubuntu: {
			...
			$dir="/var/lib/redis"
			Class ["redis::install"] -> File["$redisconfig"] -> Service["$servicename"]
			}
		freebsd: {
			if $redisname == "redis" {
				# let's default values
			...
			if $redisname != "redis" {
					...
					content => template("redis/freebsd.rc.d.erb"),
					mode => '0555',
					}
				}
			}
	...
	if $operatingsystem == "ubuntu" and $redisname != "redis" {
		file { "/etc/init/redis-$redisname.conf":
			...
			content => template("redis/ubuntu.init.erb"),
			mode => '0644',
		}
	}

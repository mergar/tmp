#!/bin/sh
# dynamic fields for plugin_name
...
BEGIN TRANSACTION;
INSERT INTO forms ( mytable,group_id,order_id,param,desc,def,cur,new,mandatory,attr,xattr,type,link,groupname ) VALUES ( "forms", ${index},${order_id},"plugin_name${index}","plugin part ${index}",'rabbitmq_stomp','','',1, "maxlen=60", "dynamic", "inputbox", "", "${groupname}" );
COMMIT;
EOF
	else
		/bin/cat <<EOF

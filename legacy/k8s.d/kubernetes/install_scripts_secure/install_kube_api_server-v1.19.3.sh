...
then
 ...
 echo "--etcd-cafile=$CERTIFICATE_MOUNT_PATH/ca.crt --etcd-certfile=$CERTIFICATE_MOUNT_PATH/$(hostname)-etcd-client.crt --etcd-keyfile=$CERTIFICATE_MOUNT_PATH/$(hostname)-etcd-client.key "
...
then
 echo "--anonymous-auth=false --authorization-mode=RBAC --admission-control=NamespaceLifecycle,LimitRanger,ServiceAccount,PersistentVolumeLabel,DefaultStorageClass,DefaultTolerationSeconds,ResourceQuota --token-auth-file=$CERTIFICATE/certs/known_tokens.csv --service-account-key-file=$CERTIFICATE/certs/server.key"
fi` \
`if [ $ENABLE_OIDC == 'true' ]
then

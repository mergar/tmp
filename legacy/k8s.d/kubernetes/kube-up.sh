...
case "${INIT_ROLE}" in
	...
	master)
		...
		max=0
		while [ ${max} -lt ${maxwait} ]; do
			_reqfile="/export/etcd.init /export/kubecertificate/certs/ca.crt /export/kubecertificate/certs/$( hostname ).crt /export/kubecertificate/certs/$( hostname )-etcd-client.crt /export/kubecertificate/certs/$( hostname )-etcd.crt"
			ready=1
	...
	worker)
		...
		max=0
		while [ ${max} -lt ${maxwait} ]; do
			_reqfile="/export/etcd.init /export/kubecertificate/certs/ca.crt /export/kubecertificate/certs/$( hostname ).crt /export/kubecertificate/certs/$( hostname )-etcd-client.crt"
			ready=1
			for i in ${_reqfile}; do
				[ ! -r ${i} ] && ready=0 && break
			done
...
if [ ! -r /export/kubecertificate/certs/ca.crt ]; then
	${ECHO} "${W1_COLOR}${MY_APP} ${MY_SHORT_HOSTNAME} error: ${N1_COLOR}no such ${N2_COLOR}/export/kubecertificate/certs/ca.crt${N0_COLOR}"
	exit 1
fi
...
case "${real_role}" in
	master)
		...
		if [ "${INIT_ROLE}" = "supermaster" ]; then
			if [ ! -r /var/lib/kubelet/kubeconfig ]; then
				${ECHO} "error: no such /var/lib/kubelet/kubeconfig!!!"
				exit 1
			fi
			# export for bscp as ubuntu user
			cp -a /var/lib/kubelet/kubeconfig /home/ubuntu/config
